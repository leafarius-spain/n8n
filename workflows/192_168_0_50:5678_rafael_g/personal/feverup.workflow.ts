...
// <workflow-map>
// Workflow : SCRAPPER FEVERUP
// Nodes   : 28  |  Connections: 32
//
// NODE INDEX
// ──────────────────────────────────────────────────────────────────
...
// ScrapeDetalleFeverup               firecrawl                  [onError→out(1)] [creds] [retry]
// FallbackDetalle                    httpRequest                [onError→regular]
// ParsearDetalleFeverup              code
...
//                                      → LoopAdjuntos (↩ loop)
//                     .out(1) → ScrapeDetalleFeverup
//                        → ParsearDetalleFeverup
//                          → ConsolidarDetalle
//                            → AceptarEspectaculo
...
//                          → ParsearDetalleFeverup (↩ loop)
//       .out(1) → FallbackListado
//          → ParsearListadoFeverup (↩ loop)
// ManualTrigger
...
export class ScrapperFeverupWorkflow {
    // =====================================================================
    // CONFIGURATION DES NOEUDS
    // =====================================================================

    @node({
        id: 'fever-schedule-trigger',
    ...
    @node({
        id: 'fever-webhook-trigger',
        webhookId: 'feverup-trigger-test',
    ...
    WebhookTrigger = {
        httpMethod: 'POST',
        path: 'feverup-trigger-test',
        responseMode: 'lastNode',
        options: {},
    };

    @node({
        id: 'fever-load-promoter-config',
...
WHERE promotor_id = 'feverup'
  AND habilitado = true;`,
        options: {},
    };

    @node({
        id: 'fever-scrape-listado',
        name: 'Scrape Listado',
    ...
    @node({
        id: 'fever-parsear-listado',
        name: 'Parsear Listado FEVERUP',
...
// Filtros (idénticos al patrón clasijazz + ubicación)
// YELMO Cines es de uso centralizado (gestionado por otro promotor), excluir.
const NOT_ESPECTACULO_BASE = /TALLER|MASTERCLASS|MASTER\\s+CLASS|WORKSHOP|CURSO|CLASE\\s+DE|INTERCAMBIO|APERITIVO|CHARLA|COLOQUIO|CONFERENCIA|MESA\\s+REDONDA|JORNADA\\s+DE\\s+CONVIVENCIA|VISITA\\s+GUIADA|RUTA\\s+GUIADA|DEPORTE|ESCAPE\\s+ROOM|ACTIVIDAD\\s+JUEGO|TARJETA\\s+REGALO|GIFT\\s+CARD|LISTA\\s+DE\\s+ESPERA|WAITING\\s+LIST|(?:NIGHT|FAMILY|SLAY|FANTASY)\\s+(?:PASS|PACK)|SPECIAL\\s+EDITION|FELIZ\\s+CUMPLE|TE\\s+QUIERO|YELMO/;
const ES_CINE = /CINE|PELICULA|FILM\\b|SCREENING|PROYECCION/;
const PRECIO_MINIMO = 12;
const LOCALIDAD = /ALMERIA|ALMER\\u00CDA/;
...
function clasificarEspectaculo(tituloNorm, locNorm, precio) {
  // Ubicación: si el venue NO contiene "Almería" -> descartar
  if (locNorm && !LOCALIDAD.test(locNorm)) return false;

  // Exclusiones absolutas (YELMO, talleres, tarjetas regalo, etc.)
  if (NOT_ESPECTACULO_BASE.test(tituloNorm)) return false;

  // Regla general: todos los eventos entran (incluido gratuitos). Las excepciones
  // (filtros de precio, cine de pago, etc.) las define el cliente sitio a sitio.
  return true;
}
...
$('.fv-plan-preview-card, .fv-plan-carousel__item').each((_, c) => {
  const $c = $(c);
  ...
  if (!idMatch) return; // sólo aceptamos cards con /m/<id> (descartamos categorías)
  const ext_id = idMatch[1];
  const event_id = 'feverup_' + ext_id;
  if (seen.has(event_id)) return;
  seen.add(event_id);
  const event_url = absoluteUrl(href);

  const titulo = clean($c.find('.fv-plan-preview-card__name').text());
  if (!titulo) return;
  const loc = clean($c.find('.fv-plan-preview-card__location').text());
  const date_text = clean($c.find('.fv-plan-preview-card__date-range').text());
  // El precio viene duplicado: "Desde 32,50 €32,50 €" -> tomamos el primero
  const priceRaw = clean($c.find('.plan-price__amount').text());
  ...
  const es_espectaculo = clasificarEspectaculo(titNorm, locNorm, precio_listado);
...
DO UPDATE SET
    ...
    @node({
        id: 'fever-select-front-sin-detalle',
...
WHERE f.source_storefront = 'feverup'
...
ORDER BY f.id;`,
    ...
    @node({
        id: 'fever-scrape-detalle',
        name: 'Scrape Detalle FEVERUP',
        type: '@mendable/n8n-nodes-firecrawl.firecrawl',
        version: 1,
        position: [416, 96],
        credentials: { firecrawlApi: { id: '3FsKPT3ZQeVfmMkM', name: 'Firecrawl account' } },
    ...
    @node({
        id: 'fever-parsear-detalle',
        name: 'Parsear Detalle FEVERUP',
        type: 'n8n-nodes-base.code',
        version: 2,
        position: [640, 96],
    })
...
if (html) {
  try {
    ...
    console.log('Parsear Detalle FEVERUP - error:', err.message);
  }
}
...
// Feverup: regla general — todos los eventos entran (incluido gratuitos y cine).
const aceptar = true;
...
return {
  json: {
    ...
    @node({
        id: 'fever-marcar-no-espectaculo',
...
DO UPDATE SET
    ...
    @node({
        id: 'fever-leer-adjuntos-pendientes',
...
WHERE d.event_id = '{{ (($('Preparar Adjunto Dropbox').item.json.event_id) || "").replace(/'/g, "''") }}';`,
    ...
    defineRouting() {
        ...
        this.ScrapeListado.out(0).to(this.ParsearListadoFeverup.in(0));
        this.ScrapeListado.out(1).to(this.FallbackListado.in(0));
        this.FallbackListado.out(0).to(this.ParsearListadoFeverup.in(0));
        this.ParsearListadoFeverup.out(0).to(this.Wait.in(0));
        ...
        this.LoopEventos.out(1).to(this.ScrapeDetalleFeverup.in(0));
        this.ScrapeDetalleFeverup.out(0).to(this.ParsearDetalleFeverup.in(0));
        this.ScrapeDetalleFeverup.out(1).to(this.FallbackDetalle.in(0));
        this.FallbackDetalle.out(0).to(this.ParsearDetalleFeverup.in(0));
        this.ParsearDetalleFeverup.out(0).to(this.ConsolidarDetalle.in(0));
